<?php 
	...
	if($count == 1 ){
		mysqli_query($con,"update tbl_cart set n_id_product='$cart_id' ,n_count='$count' where cart_id = '$cart_id' and n_id_product = '0' and n_count = '0'");
	}elseif($upcount == 1){
		$row_checkcount = mysqli_fetch_array($checkupcount);
		$countlast = $row_checkcount['n_count'];
		$countup = $row_checkcount['n_count']+$count_post;
